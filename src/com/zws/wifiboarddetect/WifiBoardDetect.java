...
public class WifiBoardDetect extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(mGetMacAddressRangeBegin.equals(MAC_NULL) || mGetMacAddressRangeEnd.equals(MAC_NULL)){
			mUser_Settings_Explain.setText("<------璇峰厛鍦ㄨ缃腑閰嶇疆MAC鍦板潃鑼冨洿----->");
		}else if(mSsid_1_Name.equals("") || mSsid_1_Name.equals(null) || mSsid_1_Level == -120){
			mUser_Settings_Explain.setText("<------璇峰厛鍦ㄨ缃腑閰嶇疆璺敱鍚嶇О鍜屾爣鍑嗗己搴�----->");
		}else if(mGetStandPercent == 100 ){
			mUser_Settings_Explain.setText("<------璇峰厛鍦ㄨ缃腑閰嶇疆淇″彿寮哄害鐧惧垎姣旈榾鍊�----->");
		}else{
		...
		mHandler = new Handler(){
			...
			public void handleMessage(Message msg){
				...
				case 0:
					...
					mDetectResult.setText("<-鏈彂鐜癢iFi妯″潡->");
					...
					if(getWifiBoardMacCommon.equalsIgnoreCase(getWifiMacRangeCommon)){ //&&
						...
						case 1:
								...
								if(percentage1 > mGetStandPercent){
									...
									mDetectResult.setText("<-Fail !!! 淇″彿寮哄害涓嶅悎鏍�->");
									mDetectResult.setTextColor(Color.RED);
								}
							break;
						case 2:
						...
						case 5:
							...
							break;
						default:
							mDetectResult.setText("<-Fail !!! 鏈悳绱㈠埌WiFi->");
							mDetectResult.setTextColor(Color.RED);
							break;
						}
					}else{
						mDetectResult.setText("<-Fail !!! Mac鍦板潃涓嶅湪鎸囧畾鑼冨洿鍐�->");
						mDetectResult.setTextColor(Color.RED);
					}
					break;
				default:
	   ...
	   /**
		...
		public boolean onKeyUp(int keyCode, KeyEvent event) { 
	        if (keyCode == KeyEvent.KEYCODE_BACK) { 
	            ...
	            if (secondTime - firstTime > 800) {//濡傛灉涓ゆ鎸夐敭鏃堕棿闂撮殧澶т簬800姣锛屽垯涓嶉��鍑� 
	                Toast.makeText(this, "鍐嶆寜涓�娆￠��鍑虹▼搴�...", 
	                        Toast.LENGTH_SHORT).show(); 
	                firstTime = secondTime;//鏇存柊firstTime 
	                return true; 
	            } else { 
	                System.exit(0);//鍚﹀垯閫�鍑虹▼搴� 
	            } 
	        } 
